...
interface CartContextType {
  ...
  updateQuantity: (id: string, format: string | undefined, delta: number) => void;
  clearCart: () => void;
  cartTotal: number;
  cartCount: number;
  isOpen: boolean;
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('kdp_press_cart');
    if (savedCart) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('kdp_press_cart', JSON.stringify(cart));
  }, [cart]);
  ...
  const updateQuantity = (id: string, format: string | undefined, delta: number) => {
    setCart((prev) =>
      prev.map((item) =>
        (item.id === id && item.format === format) ? { ...item, quantity: Math.max(1, item.quantity + delta) } : item
      )
    );
  };
